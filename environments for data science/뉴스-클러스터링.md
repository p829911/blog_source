...
def solution(str1, str2):
    ...
    ls1 = [ s for s in ls1 if len(s) > 1 ]
    ls2 = [ s for s in ls2 if len(s) > 1 ]
    
#     if (len(ls1) == 0 & len(ls2) == 0):
#         return 65536
    
    ls_result1, ls_result2 = [], []
    
    for ls in ls1:
        for i in range(len(ls)-1):
            ls_result1.append(ls[i:i+2])
    
    for ls in ls2:
        for j in range(len(ls)-1):
            ls_result2.append(ls[j:j+2])
    
    dic1 = {}
    dic2 = {}
    
    for word in ls_result1:
        try:
            dic1[word] += 1
        except:
            dic1[word] = 1
    
    for word in ls_result2:
        try:
            dic2[word] += 1
        except:
            dic2[word] = 1
    
    dic_n = set(dic1) & set(dic2)
    num_n = {}
    
    for n in dic_n:
        num_n[n] = min(dic1.get(n), dic2.get(n))
    
    dic_u = set(dic1) | set(dic2)
    num_u = {}
    ...
    for u in dic_u:
        if dic1.get(u) == None:
            num_u[u] = dic2.get(u)
        elif dic2.get(u) == None:
            num_u[u] = dic1.get(u)
        else:
            num_u[u] = max(dic1.get(u), dic2.get(u))
    
    answer = sum(num_n.values()) / sum(num_u.values()) * 65536
...
`if X and Y` 는 `X`와 `Y` 모두 판단하고 결정하지만, `if X & Y` 는 `X`를 우선 판단하고 `X` 가 0이면 False 로 결정한다.  
되도록 and를 써야겠다.
